...
ensemble_model <- function(gameIds, homeTeamIds, awayTeamIds, homeScores, awayScores, isNeutralSite){
	...
	predictByIds <- function(homeTeamId, awayTeamId, isNeutralSite = FALSE, homeSpread = 0){
		pred.bt <- model.bt$predictByIds(homeTeamId, awayTeamId, isNeutralSite, homeSpread = homeSpread)
		pred.toor <- model.toor$predictByIds(homeTeamId, awayTeamId, isNeutralSite, homeSpread = homeSpread)
		pred.gssd <- model.gssd$predictByIds(homeTeamId, awayTeamId, isNeutralSite, homeSpread = homeSpread)
		pred.zsd <- model.zsd$predictByIds(homeTeamId, awayTeamId, isNeutralSite, homeSpread = homeSpread)
		pred.prp <- model.prp$predictByIds(homeTeamId, awayTeamId, isNeutralSite, homeSpread = homeSpread)
